...
export default function StotraListScreen() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const res = await api.getStotras();
      setData(res);
      setLoading(false);
    })();
  }, []);
  ...
  return (
    <SafeAreaView style={s.root} edges={['top']}>
      ...
      <View style={s.header}>
        ...
        <Text style={s.headerTitle} numberOfLines={1}>All Stotras</Text>
        <View style={{ width: 40 }} />
      </View>
      {loading ? (
        ...
        <ScrollView contentContainerStyle={{ padding: 20 }}>
          {data.map((item, idx) => (
            <TouchableOpacity key={idx} style={s.card} onPress={() => router.push(`/stotra/${item.id}` as any)}>
              <Text style={s.god}>{item.stotra_name}</Text>
