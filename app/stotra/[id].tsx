...
export default function StotraDetailScreen() {
  ...
  useEffect(() => {
    (async () => {
      if (id) {
        setLoading(true);
        const res = await api.getStotraDetails(id as string);
        setData(res);
        setLoading(false);
      }
    })();
  ...
  if (!data) {
    return (
      <View style={[s.root, { justifyContent: 'center', alignItems: 'center' }]}>
        <Text style={{ fontSize: 18, color: C.text }}>Stotra not found.</Text>
  ...
  return (
    <SafeAreaView style={s.root} edges={['top']}>
      ...
      <View style={s.header}>
        ...
        <Text style={s.headerTitle} numberOfLines={1}>{data.stotra_name}</Text>
        <View style={{ width: 40 }} />
      </View>
      <ScrollView contentContainerStyle={{ padding: 20 }}>
        <View style={s.card}>
            ...
            {data.author ? <Text style={s.subText}><Text style={s.bold}>Author: </Text>{data.author}</Text> : null}
