...
export default function PoojaVidhiListScreen() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const res = await api.getPoojaVidhis();
      setData(res);
      setLoading(false);
    })();
  }, []);
  ...
  return (
    <SafeAreaView style={s.root} edges={['top']}>
      ...
      <View style={s.header}>
        ...
        <Text style={s.headerTitle} numberOfLines={1}>All Pooja Vidhis</Text>
        <View style={{ width: 40 }} />
      </View>
      {loading ? (
        ...
        <ScrollView contentContainerStyle={{ padding: 20 }}>
          {data.map((item, idx) => (
            <TouchableOpacity key={idx} style={s.card} onPress={() => router.push(`/pooja_vidhi/${item.id}` as any)}>
              <Text style={s.god}>{item.vidhi_name}</Text>
