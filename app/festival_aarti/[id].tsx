...
export default function FestivalAartiDetailScreen() {
  ...
  useEffect(() => {
    (async () => {
      if (id) {
        setLoading(true);
        const data = await api.getFestivalAartiDetails(id as string);
        setMantraData(data);
        setLoading(false);
      }
    })();
  ...
  if (!MANTRA_DATA) {
    return (
      <View style={[s.root, { backgroundColor: C.bg, justifyContent: 'center', alignItems: 'center' }]}>
        <Text style={{ fontSize: 18, color: C.text }}>Festival Aarti not found.</Text>
      ...
      <Animated.View style={[s.header, { height: headerHeight, paddingTop: insets.top }]}>
        ...
        <Animated.View style={[s.headerExpanded, { opacity: headerOpacity }]}>
          <View style={s.categoryBadge}>
            <Text style={s.categoryBadgeText}>{MANTRA_DATA.category || 'Festival Aarti'}</Text>
          </View>
